...
\frame{\frametitle{Generalized Additive Model}
\begin{figure}[h]
\centering
\includegraphics[width = 90mm]{figs/gam.pdf}
\end{figure}
\begin{itemize}
\item Regression splines: 
$$s_{j}(x) \> =  \> \sum_{j,k} \beta_{jk} {\phi_{jk}}(x) = {\boldsymbol{\beta{'}}}{\phi_j}$$
\item Estimate ${\cub \boldsymbol\beta}$ by minimizing the penalized least squares objective function. 
\end{itemize}
...
\begin{itemize} 
\setlength\itemsep{1.5em}
\item {\cub Formula}: $\PM$ $\sim$ AOD + s(windspeed) $+$ s(winddirection) $+$ s(hum) $+$ s(airtemp) $+$ season
\item {\cub Coefficient} = 4.085, ${\cub \beta_{AOD}}$ = -2.283 (t = 3.84, p-value = 0.0023). 
\item ${\cub R^2}$ = 0.782, {\cub Deviance} = $81.4\%$
\item We compared various models using a subset of covariates and chose the model with the smallest {\cub AIC} (Akaike's Information Criterion). 
\end{itemize} 
}
...
\end{figure}
\begin{itemize}
\item {\cub Equivalent degrees of freedom} = (2.69, 2.54, 2.37, 3.00)
\end{itemize}
}
...
\bibitem{lee} Lee, H. J., et al. ``A novel calibration approach of MODIS AOD data to predict PM2. 5 concentrations.'' Atmos. Chem. Phys 11.15 (2011): 7991-8002.

\bibitem{van} van Donkelaar, Aaron, et al. ``Use of satellite observations for long-term exposure assessment of global concentrations of fine particulate matter.'' Diss. University of British Columbia, 2015.

\bibitem{li} Li, Jing, Barbara E. Carlson, and Andrew A. Lacis. ``How well do satellite AOD observations represent the spatial and temporal variability of PM 2.5 concentration for the United States?.'' Atmospheric Environment 102 (2015): 260-273.

\bibitem{liu} Liu, Yang, Christopher J. Paciorek, and Petros Koutrakis. ``Estimating Regional Spatial and Temporal Variability of PM$_{2.5}$ Concentrations Using Satellite Data, Meteorology, and Land Use Information.'' Environmental health perspectives 117.6 (2009): 886. 
